...
module.exports = Platform = (function() {
  function Platform(state, x, y, type, noStars) {
    ...
    speed = this.game.tutorial ? 0 : -CFG.platformSpeed;
    this.sprite.body.velocity.setTo(speed, 0);
    this.sprite["class"] = this;
    this.spikes = this.game.add.sprite(x, y, 'spikes');
    this.spikes.anchor.setTo(0, 1);
    this.spikes.alpha = type.name === "spiked" ? 1 : 0;
    this.star = new Star(state, this, noStars);
  ...
  Platform.prototype.reset = function(x, y, type, userInput, noStars) {
    if (x == null) {
      x = this.game.width;
    }
    if (y == null) {
  ...
  Platform.prototype.getRandomY = function() {
    return this.game.rnd.integerInRange(100, this.game.height - 180);
  };

  Platform.prototype.getRandomType = function() {
    var key, random, sum, tmp;
    sum = 0;
...
module.exports = Platforms = (function() {
  ...
  Platforms.prototype.generatePlatforms = function() {
    ...
    if (rightmost.sprite.x < this.game.width - 250) {
      return this.requestPlatform();
    }
  };

  Platforms.prototype.initPlatforms = function() {
    this.requestPlatform(0, 300, CFG.platformTypes.normal, false, true);
    this.requestPlatform(150, 300, CFG.platformTypes.normal, false, true);
    this.requestPlatform(300, 300, CFG.platformTypes.spiked, false, true);
    return this.requestPlatform(450, 300, CFG.platformTypes.normal, false, true);
  };
  ...
  Platforms.prototype.requestPlatform = function(x, y, type, userInput, noStars) {
    ...
    if (leftmost) {
      return leftmost.reset(x, y, type, userInput, noStars);
    } else {
...
module.exports = Player = (function() {
  function Player(state) {
    this.game = state.game;
    this.state = state;
    this.sprite = this.game.add.sprite(0, this.game.height * .5, 'dude');
    this.game.physics.arcade.enable(this.sprite);
...
module.exports = Star = (function() {
  function Star(state, platform, noStars) {
    var spawns;
    this.game = state.game;
    this.relX = platform.sprite.body.halfWidth;
    this.relY = platform.sprite.body.height;
    this.platform = platform;
    this.sprite = this.game.add.sprite(platform.sprite.body.x + this.relX, platform.sprite.body.y - this.relY, 'star');
    this.sprite.anchor.setTo(.5, 1);
    spawns = !noStars && this.platform.type.starChance && this.game.rnd.integerInRange(0, 100) <= platform.type.starChance;
...
module.exports = {
  preload: function() {
    ...
    return this.game.load.spritesheet('dude', 'assets/dude2.png', 73, 133);
  },
  init: function() {
...
module.exports = {
  create: function() {
    var lost, restart, score;
    lost = this.game.add.text(this.game.world.width / 2, this.game.world.height * .4, 'You lost', {
    ...
    restart = this.game.add.text(this.game.world.width / 2, this.game.world.height * .75, 'tap to restart...', {
...
module.exports = {
  init: function() {
    ...
    return this.game.tutorial = true;
  },
  create: function() {
...
module.exports = UI = (function() {
  function UI(state) {
    ...
    this.frame.position.setTo(0, this.game.height - this.frame.height);
    this.fps = this.game.add.text(this.game.width, this.game.height, '0 fps', {
      font: '16px sans-serif',
      fill: '#FF0'
    });
    this.fps.anchor.setTo(1, 1);
    this.tutorialText = this.game.add.text(this.game.width / 2, this.game.height - this.frame.height, 'Begin with dragging the platform on the playground', {
      font: '16px sans-serif',
      fill: '#FF0'
    });
    this.tutorialText.anchor.setTo(.5, 0);
    this.coins = this.game.add.text(this.game.width / 2, 0, '', {
      font: '16px sans-serif',
      fill: '#FF0'
    });
    this.coins.anchor.setTo(0.5, 0);
    this.score = this.game.add.text(this.game.width, 0, '', {
      font: '16px sans-serif',
      fill: '#FF0'
    });
    ...
    this.dragging = false;
    this.firstDrag = true;
    this.platformBtn = this.game.add.sprite(0, 0, 'platform-btn');
    this.platformBtn.anchor.setTo(.5, .5);
    this.platformBtn.position.setTo(this.platformBtn.width, this.frame.position.y + this.frame.height / 2);
    this.platformBtn.inputEnabled = true;
    this.game.input.onUp.add(function() {
      if (this.dragging && this.game.input.y < this.game.ui.frame.position.y - this.draggedPlatform.height) {
      ...
      this.dragging = false;
      return this.draggedPlatform.position.setTo(this.game.width, this.game.height);
    }, this);
  ...
  UI.prototype.update = function() {
    ...
    this.score.setText('Score: ' + parseInt(this.game.score));
    this.coins.setText('Stars: ' + this.game.starCount);
    if (this.dragging) {
      return this.draggedPlatform.position.setTo(this.game.input.x, this.game.input.y);
    }
  };

  UI.prototype.onTap = function() {
    if (this.game.paused) {
      this.game.paused = false;
      this.overlay.destroy();
      this.resumeText.destroy();
      return this.pauseText.destroy();
    } else if (this.isInputOver(this.platformBtn)) {
      this.dragging = true;
      return this.draggedPlatform.position.setTo(this.game.input.x, this.game.input.y);
      ...
      this.resumeText = this.game.add.text(this.game.width / 2, this.game.height * .50, 'tap to resume...', {
        font: '16px sans-serif',
        fill: '#FFF'
      });
  ...
  UI.prototype.onFirstDrag = function() {
    ...
    this.game.world.remove(this.tutorialText);
